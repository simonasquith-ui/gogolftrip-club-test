...
exports.handler = async (event) => {
  ...
  try {
    ...
    // IMPORTANT: month-matrix does NOT support one_way / trip_duration at all —
    // those params only exist on the separate v2/prices/latest endpoint, and
    // Travelpayouts silently ignores unknown params. So every price this
    // endpoint returns is a ONE-WAY fare, regardless of what we pass. We used
    // to send one_way=false + trip_duration here thinking it made this
    // round-trip — confirmed via Travelpayouts' own API docs that it does
    // nothing, so removed to avoid implying a capability that isn't there.
    // The client (index.html) is responsible for calling this endpoint twice
    // (once with origin/destination as-is for the outbound leg, once swapped
    // for the return leg) and combining the two one-way fares itself.
    let url = 'https://api.travelpayouts.com/v2/prices/month-matrix' +
